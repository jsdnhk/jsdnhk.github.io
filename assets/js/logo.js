...
const title_quote_complete =
  "Great person, you've reviewed all the quotes in this month!";
const title_quote_complete2 =
  "That the saints will bring you to the bright after dark!:-)";
...
function getQuoteMessage(json) {
  var quote_message = "";
  if (json) {
    obj_selected_json = json;
  }
  var index_select_quote = getRandomQuoteIndex(obj_selected_json.count);
  if (index_select_quote != null) {
    var text_select_quote = obj_selected_json.results[
      index_select_quote
    ].quoteText
      .toString()
      .trim();
    var author_select_quote = obj_selected_json.results[
      index_select_quote
    ].quoteAuthor
      .toString()
      .trim();
...
function getRandomQuoteIndex(count_total) {
  ...
  if (array_quote_indexes.length == 0) {
    return null;
  }
...
function getPrintedQuoteMessage(quote_message) {
  ...
  if (quote_message) {
    if (count_quotes_total - count_quotes_read > 0) {
      str_printed = `(${
        count_quotes_total - count_quotes_read
      }) ${title_quote}`;
    } else {
